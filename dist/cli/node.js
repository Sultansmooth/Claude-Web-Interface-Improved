  ...
  try {
    ...
    for await (const sdkMessage of query({
      ...
      options: {
        ...
        appendSystemPrompt: "IMPORTANT: When you need to ask the user a question with options, use the mcp__ask-user-webui__AskUserQuestion tool. This is the correct tool for asking user questions in this web interface.\n\nCRITICAL GIT RULES:\n- If git commit fails with 'Please tell me who you are' or missing user.name/user.email: DO NOT retry. Ask the user for their name and email using mcp__ask-user-webui__AskUserQuestion, then set git config (without --global) and retry ONCE.\n- If git push fails for ANY reason (auth, network, rejected, etc.): NEVER retry git push. Tell the user what went wrong and ask how they want to proceed. Retrying push will not fix auth or network issues.\n- If git pull/fetch fails: NEVER retry. Report the error to the user.\n\nCRITICAL ANTI-LOOP RULES (after context compaction):\n- NEVER respond with just 'No response requested.' - this is ALWAYS wrong in this web UI. The user is always waiting for you to do something.\n- After context compaction (when you see a summarized conversation), you MUST continue working on the task described in the summary. Re-read the summary carefully and pick up where you left off.\n- If the user says 'continue' or 'Continue from where you left off', they are asking you to resume the task. DO NOT say 'No response requested.' Instead, identify the last incomplete task from the conversation summary and continue it.\n- If you are unsure what to do after compaction, ask the user what they'd like you to work on using mcp__ask-user-webui__AskUserQuestion rather than doing nothing."
      }
    })) {
